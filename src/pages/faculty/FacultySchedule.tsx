...
export default function FacultySchedule() {
    ...
    useEffect(() => {
        const fetch = async () => {
            ...
            } catch (err) { console.error(err); toast.error('Failed to load schedule'); }
            setLoading(false);
        };
        fetch();
    }, [user]);
            ...
            <div className="max-w-7xl mx-auto space-y-8 animate-fade-in pb-12">
                <div>
                    <div className="text-sm text-muted-foreground mb-2 flex items-center gap-2">
                        <span>Faculty</span><span>/</span><span>Duties</span><span>/</span><span className="text-foreground font-medium">Schedule</span>
                    </div>
                    <h1 className="text-3xl font-display font-bold text-foreground mb-2">My Schedule</h1>
                    <p className="text-muted-foreground">View your upcoming invigilation duties.</p>
                </div>
                            ...
                            return (
                                <Card key={d.id} className="dashboard-card">
                                    <CardHeader className="border-b pb-4">
                                        ...
                                        <p className="text-xs text-muted-foreground">{exam?.subjectCode} - {exam?.subjectName}</p>
                                    </CardHeader>
                                    <CardContent className="pt-4 space-y-2">
                                        <p className="text-sm"><span className="font-semibold">Date:</span> {exam?.date || 'TBD'}</p>
